"use client";
import * as React from "react";
import { Bold, Menu, X } from "lucide-react";
import { SlArrowDown } from "react-icons/sl";

export default function MenuTrying() {
  const [menuOpen, setMenuOpen] = React.useState<boolean>(false);
  const [featureOpen, setFeatureOpen] = React.useState<boolean>(false);
  const [aboutOpen, setAboutOpen] = React.useState<boolean>(false);
  const [courseOpen, setCourseOpen] = React.useState<boolean>(false);

  const allFeatures=()=>{
    setFeatureOpen((prev)=>!prev);
  }

  const allAbout=()=>{
    setAboutOpen((prev)=>!prev);
  }

  const allCourses=()=>{
    setCourseOpen((prev)=>!prev);
  }

  // --- PROBLEM FIX 2: Page scroll lock karna ---
  React.useEffect(() => {
    if (menuOpen) {
      document.body.style.overflow = "hidden"; // Scroll band
    } else {
      document.body.style.overflow = "auto"; // Scroll chalu
    }
  }, [menuOpen]);

  return (
    <nav className="relative">
      <div className="flex justify-between items-center shadow-md p-5 bg-white z-[60] fixed w-full top-0 md:hidden">
        <h1 className="text-xl font-bold text-blue-600">HelloTalk</h1>
        <button onClick={() => setMenuOpen(!menuOpen)} className="p-2">
          {menuOpen ? <X size={28} /> : <Menu size={28} />}
        </button>
      </div>

      {/* Overlay */}
      <div
        className={`fixed inset-0 bg-black/50 transition-opacity duration-300 z-40 ${
          menuOpen ? "opacity-100 backdrop-blur-sm visible" : "opacity-0 invisible"
        }`}
        onClick={() => setMenuOpen(false)}
      />

      {/* --- PROBLEM FIX 1: Menu Scrollable Container --- */}
      <div
        className={`fixed top-0 left-0 w-full h-screen bg-white z-50 shadow-2xl transform transition-transform duration-300 ease-in-out overflow-y-auto ${
          menuOpen ? "translate-y-0" : "-translate-y-full" 
        }`}
      >
        <div className="p-6 pt-24 pb-10"> {/* pt-24 taaki header ke niche se shuru ho */}
          <ul className="flex flex-col gap-6 text-lg font-medium text-gray-700">
            {/* Aapke saare menu items yahan */}
            <h3 className="font-bold flex items-center justify-between" onClick={allFeatures}>All Features <SlArrowDown size={15}/></h3>
           {featureOpen &&(
            <ul className="space-y-4 ml-4 border-l-2 border-gray-300 pl-4">
            <li className="border-b pb-2"><a href="./home">Home</a></li>
            <li className="border-b pb-2"><a href="./about">About</a></li>
            <li className="border-b pb-2"><a href="./contact">Contact</a></li>
            <li className="border-b pb-2"><a href="./help">Help</a></li>
            </ul>
            )}
            <h3 className="font-bold flex items-center justify-between" onClick={allAbout}>All About <SlArrowDown size={15}/></h3>
            {aboutOpen &&(
            <ul className="space-y-4 ml-4 border-l-2 border-gray-300 pl-4">
           <li className="border-b pb-2"><a href="./about-business">About Business</a></li>
            <li className="border-b pb-2"><a href="./about-services">About Services</a></li>
            <li className="border-b pb-2"><a href="./about-us">About Us</a></li>
            <li className="border-b pb-2"><a href="./about-advertisement">About Advertisement</a></li>
            <li className="border-b pb-2"><a href="./about-learning">About Learning</a></li>
            <li className="border-b pb-2"><a href="./about-duration">About Duration</a></li>
            </ul>
            )}
            <h3 className="font-bold flex items-center justify-between" onClick={allCourses}>All Courses <SlArrowDown size={15}/></h3>
            {courseOpen &&(
            <ul className="space-y-4 ml-4 border-l-2 border-gray-300 pl-4">
            <li className="border-b pb-2"><a href="./about-courses">About Courses</a></li>
            <li className="border-b pb-2"><a href="./about-pro">About Professionalism</a></li>
            </ul>
            )}
            <li className="border-b pb-2"><a href="./about-courses">About Courses</a></li>
            <li className="border-b pb-2"><a href="./about-pro">About Professionalism</a></li>
          </ul>
          
          <button className="w-full mt-8 py-4 bg-blue-600 text-white rounded-xl font-bold">
            Get Started Now
          </button>
        </div>
      </div>
    </nav>
  );
}
